<?php
/**
 * Completed Course on Category
 */
if ( ! class_exists( 'myCRED_Sensei_Completed_Course_Category_Hook' ) ) :
class myCRED_Sensei_Completed_Course_Category_Hook extends myCRED_Hook {
	...
	function __construct( $hook_prefs, $type ) {
		...
		$defaults = array(
			array(
				'course_cat_id'   => 'any',
				'creds'   => 1,
				'log'     => __('%plural% for completed course on category', MYCRED_SENSEI ),
				'limit'  => '0/x'
			)
		);

		if ( isset( $hook_prefs['completed_course_category'] ) )
			$defaults = $hook_prefs['completed_course_category'];

		parent::__construct( array(
			'id'       => 'completed_course_category',
			'defaults' => $defaults
		), $hook_prefs, $type );
	}
	 ...
	 */
	public function run() {
		add_action( 'sensei_user_course_end', array($this, 'do_log'), 10, 2 );
	}
	...
	public function do_log($user_id, $course_id){
		...
		if(in_array('any', $course_cat_ids)){
			...
			// check limit
			if ($this->over_hook_limit( $instance_any, 'completed_course_category', $user_id )) $need_process = false;
		...
		if(!empty($matched) && is_array($matched)){
			foreach ($matched as $matchkey => $course_cat_id) {
				...
				//check limit
				if ($this->over_hook_limit( $instance, 'completed_course_category', $user_id )) $need_process = false;
		...
		if ( $this->core->has_entry( 'completed_course_category', $course_id, $user_id, $data, $this->mycred_type ) ) return;

		// Check creds
		if ( $creds > 0 ){
			$this->core->add_creds(
				'completed_course_category',
				$user_id,
	 ...
	 */
	 public function preferences() {
		...
		<div class="container-setting" style="padding: 10px 0;">
			...
			<div class="repeatable-container" data-template="#template_completed_course_category"></div>
	...
	public function get_repeatable_template($course_cats){
		...
		<!-- template -->
		<script type="text/template" id="template_completed_course_category">
			<div class="field-group" style="position: relative; padding-left: 50px; ">
			    <!-- select course -->
				<label class="subheader"><?php _e('Course Category', MYCRED_SENSEI); ?></label>
				<ol>
