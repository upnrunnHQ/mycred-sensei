 ...
 * Completed Course
 */
if ( ! class_exists( 'myCRED_Sensei_Completed_Course_Hook' ) ) :
class myCRED_Sensei_Completed_Course_Hook extends myCRED_Hook {
	...
	function __construct( $hook_prefs, $type ) {
		...
		$defaults = array(
			array(
				'course_id'   => 'any',
				'creds'   => 1,
				'log'     => __('%plural% for completed course', MYCRED_SENSEI ),
				'limit'  => '0/x'
			)
		);

		if ( isset( $hook_prefs['completed_course'] ) )
			$defaults = $hook_prefs['completed_course'];
	...
	public function do_log($user_id, $course_id){
		...
		if(in_array('any', $course_ids)){
			...
			// check limit
			if ($this->over_hook_limit( $instance_any, 'completed_course', $user_id )) $need_process = false;
		...
		if(in_array($course_id, $course_ids)){
			...
			if ($this->over_hook_limit( $instance, 'completed_course', $user_id )) $need_process = false;
		...
		if ( $this->core->has_entry( 'completed_course', $course_id, $user_id, $data, $this->mycred_type ) ) return;
	 ...
	 */
	 public function preferences() {
		...
		<div class="container-setting" style="padding: 10px 0;">
			...
			<div class="repeatable-container" data-template="#template_completed_course"></div>
	...
	public function get_repeatable_template($courses){
		...
		<!-- template -->
		<script type="text/template" id="template_completed_course">
			<div class="field-group" style="position: relative; padding-left: 50px; ">
			    <!-- select course -->
				<label class="subheader"><?php _e('Course', MYCRED_SENSEI); ?></label>
				<ol>
