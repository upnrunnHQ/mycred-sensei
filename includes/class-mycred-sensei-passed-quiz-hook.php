<?php
/**
 * Enrolled passed quiz
 */
if ( ! class_exists( 'myCRED_Sensei_Passed_Quiz_Hook' ) ) :
class myCRED_Sensei_Passed_Quiz_Hook extends myCRED_Hook {
	...
	function __construct( $hook_prefs, $type ) {
		...
		$defaults = array(
			array(
				'quiz_id'   => 'any',
				'creds'   => 1,
				'log'     => __('%plural% for passed quiz', MYCRED_SENSEI ),
				'limit'  => '0/x'
			)
		);

		if ( isset( $hook_prefs['passed_quiz'] ) )
			$defaults = $hook_prefs['passed_quiz'];
	...
	public function do_log($user_id, $quiz_id, $grade, $quiz_passmark, $quiz_grade_type){
		...
		if(in_array('any', $quiz_ids)){
			...
			// check limit
			if ($this->over_hook_limit( $instance_any, 'passed_quiz', $user_id )) $need_process = false;
		...
		if(in_array($quiz_id, $quiz_ids)){
			...
			if ( empty( $this->prefs[ $instance ]['creds'] ) || $this->prefs[ $instance ]['creds'] == 0 ) $need_process = false;
 			...
 			if ($this->over_hook_limit( $instance, 'passed_quiz', $user_id )) $need_process = false;
		...
		if ( $this->core->has_entry( 'passed_quiz', $quiz_id, $user_id, $data, $this->mycred_type ) ) return;
	...
	public function get_replaced_log($instance, $quiz_id){
		...
		return str_replace('%quiz_title%', $replace, $this->prefs[ $instance ]['log']);
	}
	 ...
	 */
	 public function preferences() {
		...
		<div class="container-setting" style="padding: 10px 0;">
			...
			<div class="repeatable-container" data-template="#template_passed_quiz"></div>
	...
	public function get_repeatable_template($quizes){
		...
		<!-- template -->
		<script type="text/template" id="template_passed_quiz">
			<div class="field-group" style="position: relative; padding-left: 50px; ">
			    <!-- select quiz -->
				<label class="subheader"><?php _e('Quiz', MYCRED_SENSEI); ?></label>
				<ol>
